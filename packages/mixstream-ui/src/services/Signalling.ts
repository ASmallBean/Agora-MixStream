...
import { SignalCommand, SignalPayload } from 'mixstream-shared';
...
export class Signalling extends EventEmitter {
  ...
  private bindEvents() {
    ...
    this.rtmClient.on('ConnectionStateChanged', this.handleConnectionStateChange);
    this.rtmChannel.on('ChannelMessage', this.handleEvents);
  }

  private unbindEvents() {
    this.rtmClient.off('MessageFromPeer', this.handleEvents);
    this.rtmClient.off('ConnectionStateChanged', this.handleConnectionStateChange);
